...
// socket.io의 소켓이 http 모듈로 생성된 서버에서만 동작
...
io.on('connection', socket => {
  ...
  socket.on('event_name', (arg1, arg2, arg3, cb) => {
    ...
    cb(arg1, arg2, arg3); //emit 에게 다시 데이터 전달
  });

  // 2. client > server
  socket.on('cb_test', (arg, cb) => {
    console.log('[cb_test.arg]::', arg);
  ...
  socket.on('new_message1', (arg, cb) => {
    // 그대로 보내져오기 때문에 파서등의 걱정 안해도 됨
    console.log('[new_message1]::', arg);
    cb(arg); // 나에게만 전달함.
  });

  // ver.2 나의 메세지가 모두에게 보이도록
  ...
  socket.on('study', msg => {
    console.log('client :', msg);
    socket.emit('study2', msg); // 전체 클라이언트에게 보낼경우 io.emit()으로 작성
  });
  ...
  // ========== 방 있는 채팅 =============
  //   console.log('socket rooms >>', socket.rooms); // Set(1) { 'S76kplACMZXehKviAAAB' }
  ...
  socket.on('join', roomname => {
    ...
    socket.join(roomname); // 방열기기
    socket.room = roomname; // 다른곳에서도 roomname을 확인할 수 있도록 정보를 추가
    // console.log('socket.rooms', socket.rooms); // socket.rooms Set(2) { '029NCE6DENOQtSzEAAAF', '123' }

    // 3-1. 입장메세지를 모두에게 보내기 (server > client)
