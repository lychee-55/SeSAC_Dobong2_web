...
export default function Chatting1() {
  ...
  useEffect(() => {
    ...
    // [문제점 1] : newChatList 를 만들 때, ChatList가 mount된 시점의 chatList!! 쌓이는게 아니라 새로운 것만 추가함.
    ...
    //   //     return [...prevState, { type: 'notice', content: notice }];
    //   //   });
    // });
    // 방법1: prevState
    // socket.on('notice', notice => {
  ...
  useEffect(() => {
    // chatList가 변경될 때마나 이벤트 핸들러 재등록
    // 기존 chatList가 기준이 아닌 변경된 chatList가 기준이 될 수 있도록

    // [해결 1] >> [문제점 2] : 이벤트 헨들러 재등록록
    // socket.on('notice', notice => {
    //   const newChatList = [
    //     ...chatList /* 항상 초기값 배열 */,
